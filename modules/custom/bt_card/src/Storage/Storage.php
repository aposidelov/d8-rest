...
namespace Drupal\bt_card\Storage;
...
class Storage {
  ...
  public static function insert($entry) {
    $return_value = NULL;
    try {
      $return_value = Database::getConnection()->insert('bt_card')
          ->fields($entry)
          ->execute();
    }
    catch (\Exception $e) {
        MessengerInterface::addMessage(t('db_insert failed. Message = %message, query= %query', array(
  ...
  public static function update($entry) {    
    try {
      ...
      $count = Database::getConnection()->update('bt_card')
          ->fields($entry)
          ->condition('id', $entry['id'])
          ->execute();
    }
    catch (\Exception $e) {
      MessengerInterface::addMessage(t('db_update failed. Message = %message, query= %query', array(
            '%message' => $e->getMessage(),
            '%query' => $e->query_string,
          )), 'error');
    }
   ...
   *
   * @param array $entry
   *   An array containing at least the product identifier 'id' element of the
   *   entry to delete.
   *
   * @see db_delete()
   */
  public static function delete($entry) {
      Database::getConnection()->delete('bt_card')
        ->condition('id', $entry['id'])
        ->execute();
  }
   ...
   *
   * Drupal provides an abstracted interface that will work with a wide
  ...
  public static function load($entry = array()) {
    ...
    $select = Database::getConnection()->select('bt_card', 'c');
    $select->fields('c');
  ...
  public static function advancedLoad() {
    ...
    $select->addField('c', 'title');
    $select->addField('c', 'price');
    $select->addField('c', 'sku');

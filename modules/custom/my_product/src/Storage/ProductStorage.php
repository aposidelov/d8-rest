...
namespace Drupal\my_product\Storage;
...
class ProductStorage {
  ...
  public static function insert($entry) {
    $return_value = NULL;
    try {
      $return_value = db_insert('my_product')
          ->fields($entry)
          ->execute();
    }
    catch (\Exception $e) {
  ...
  public static function advancedLoad() {
    ...
    $select->join('users_field_data', 'u', 'p.uid = u.uid');
    // Select these specific fields for the output.
    $select->addField('p', 'pid');
    $select->addField('u', 'name', 'username');
    $select->addField('p', 'title');
    $select->addField('p', 'price');
    $select->addField('p', 'sku');
